...
// v2: el código se envía por WhatsApp Cloud API DIRECTO, sin plataforma
// intermediaria (antes lo disparaba un Zap).
...
exports.handler = async (evento) => {
  ...
  try {
    ...
    if (usuarios.length === 1) {
      ...
      const tel = u.fields[L.C.U_TEL];
      if (tel) {
        // Plantilla de AUTENTICACIÓN preaprobada en Meta (es_MX)
        await W.plantilla(tel, "sscae_codigo_acceso", [codigo]).catch(() => {});
      }
    }
