...
// de sesión firmada (24 h). Bloqueo a partir de 5 intentos (§4.2).
"use strict";
...
exports.handler = async (evento) => {
  ...
  try {
    ...
    const hashOk = u.fields[L.C.U_HASH] && L.igualSeguro(u.fields[L.C.U_HASH], L.sha256(codigo));
    const vigente = expira && Date.now() < expira.getTime();

    if (!hashOk || !vigente) {
      await L.airtable("PATCH", L.T.USUARIOS, {
        id: u.id, cuerpo: { fields: { [L.C.U_INTENTOS]: intentos + 1 } }
      }).catch(() => {});
      return RECHAZO;
    }

    await L.airtable("PATCH", L.T.USUARIOS, { id: u.id, cuerpo: { fields: {
      [L.C.U_HASH]: "", [L.C.U_INTENTOS]: 0, [L.C.U_ULTIMO]: new Date().toISOString()
    } } });
