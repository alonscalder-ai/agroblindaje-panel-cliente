...
// Devuelve SOLO los campos declarados en el pliego; jamás el registro completo.
"use strict";
const L = require("./_lib.js");

exports.handler = (evento) => L.conSesion(evento, async (usuario) => {
  const q = evento.queryStringParameters || {};
  const filtros = [L.filtroClientePorLookup(usuario.clienteId), L.filtroRol(usuario.rol)];
  ...
  const registros = await L.listar(
    ...
    [L.C.O_FOLIO, L.C.O_COMPRADOR, L.C.O_AVAL, L.C.O_SALDO, L.C.O_F_VENC,
     ...
     L.C.O_CARGO_ACTIVO, L.C.O_RESPONSABLE],
    { "sort[0][field]": L.C.O_F_VENC, "sort[0][direction]": "asc" }
  );

  let tarjetas = L.filtrarPorRol(registros, usuario.rol, usuario.id).map(r => {
    const f = r.fields;
    return {
      folio: f[L.C.O_FOLIO] || "",
