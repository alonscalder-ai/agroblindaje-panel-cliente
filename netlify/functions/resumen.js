...
// Caché de 60 s por cliente+rol. Incluye bandera de modo histórico (§4.4).
"use strict";
  ...
  const datos = await L.conCache(`resumen:${usuario.clienteId}:${usuario.rol}`, 60e3, async () => {
    const [cliente, ops] = await Promise.all([
      ...
      L.listar(L.T.OPERACIONES,
        `AND(${L.filtroClientePorLookup(usuario.clienteId)}, ${L.filtroRol(usuario.rol)})`,
        [L.C.O_SALDO, L.C.O_F_VENC, L.C.O_ESTATUS_PAGO,
         L.C.O_FICHA_FIRMA, L.C.O_PAGARE_FIRMA, L.C.O_CONST_ESTATUS, L.C.O_RESPONSABLE])
    ]);
    const propias = L.filtrarPorRol(ops, usuario.rol, usuario.id);
    ...
    const paquete = cliente.fields[L.C.CL_PAQUETE];
    return {
      cliente: cliente.fields[L.C.CL_NOMBRE] || "",
      logo: logo ? logo.url : null,
      paquete: Array.isArray(paquete) ? paquete[0] : (paquete || ""),
      modoHistorico,
