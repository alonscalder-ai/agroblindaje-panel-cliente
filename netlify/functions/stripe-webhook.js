...
async function operacionPorFolio(folio) {
  const regs = await L.listar(L.T.OPERACIONES,
    ...
    [L.C.O_FOLIO, L.C.O_BITACORA, L.C.O_SALDO, L.C.O_ESTATUS_PAGO, L.C.O_CARGOS_FALLIDOS]);
  ...
  try {
    ...
    switch (ev.type) {
      ...
      case "invoice.payment_succeeded": {
        ...
        const cubreTodo = monto / 100 >= saldo - 0.01;
        // El cobro domiciliado exitoso SÍ liquida: es el único evento con esa
        // facultad (regla de conciliación del Árbol §5).
        if (cubreTodo) campos[L.C.O_ESTATUS_PAGO] = "Liquidada";
        campos[L.C.O_CARGOS_FALLIDOS] = 0;
        texto = `Cobro domiciliado exitoso por ${pesos(monto)}` +
