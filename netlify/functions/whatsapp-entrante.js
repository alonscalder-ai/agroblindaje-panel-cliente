...
async function contrapartesRecientes(clienteId) {
  const regs = await L.listar(L.T.OPERACIONES,
    ...
    [L.C.O_COMPRADOR, L.C.O_COMPRADOR_RFC, L.C.O_COMPRADOR_DOM,
     L.C.O_COMPRADOR_TEL, L.C.O_COMPRADOR_CORREO, L.C.O_COMPRADOR_REP],
    { "sort[0][field]": L.C.O_FOLIO, "sort[0][direction]": "desc" });
  const vistos = new Map();
  for (const r of regs) {
    const n = r.fields[L.C.O_COMPRADOR];
    if (n && !vistos.has(n)) vistos.set(n, r.fields);
    if (vistos.size >= 9) break;
  }
  return [...vistos.entries()].map(([nombre, f]) => ({ nombre, f }));
}
  ...
  switch (paso) {
    case "ALTA_CONTRAPARTE": {
      if (entrada === "CP_NUEVO") {
        ...
        return W.texto(tel,
          "Para un comprador nuevo necesito sus datos completos (razón social, RFC, " +
          "domicilio, teléfono y correo). Llénalos una sola vez en este formulario y " +
          "al terminar escribe *ALTA* para continuar:\n\n" +
          (process.env.FORM_CONTRAPARTE_URL || "(formulario pendiente de configurar)"));
      }
    ...
    case "ALTA_AVAL": {
      if (entrada === "AVAL_SI") {
        await limpiarSesion(sesion, tel, usuario.id);
        return W.texto(tel,
          "El aval necesita cinco datos más su correo (para poder invitarlo a firmar). " +
          "Es demasiado tecleo para el chat: llénalos aquí y al terminar escribe *ALTA* " +
          "para retomar.\n\n" +
          (process.env.FORM_AVAL_URL || "(formulario pendiente de configurar)"));
      }
...
async function confirmarAlta(ctx) {
  ...
  const recientes = await L.listar(L.T.OPERACIONES,
    `AND(${L.filtroClientePorLookup(usuario.clienteId)}, {${C.O_COMPRADOR}} = ${L.escFormula(cp[C.O_COMPRADOR])}, {${C.O_MONTO_TOTAL}} = ${datos.montoTotal})`,
    [C.O_FOLIO, "Creado"]).catch(() => []);
  const duplicado = recientes.some(r => {
    const t = r.fields.Creado ? Date.parse(r.fields.Creado) : 0;
    return t && Date.now() - t < 10 * 60e3;
  });
  ...
  if (clienteFields.__contratoId) fields[C.O_CONTRATO] = [clienteFields.__contratoId];
