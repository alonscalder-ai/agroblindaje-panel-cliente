...
(function () {
  ...
  function tarjetaHTML(t) {
    const s = t.semaforo;
    const extra = s.clave === "vencida"
      ? ` · ${s.diasMora} día${s.diasMora === 1 ? "" : "s"} de mora`
    ...
    return `<button type="button" class="tarjeta" data-sem="${s.clave}" data-folio="${esc(t.folio)}">
      ...
      <div class="puntos-exp">
        ...
        &nbsp;·&nbsp;${esc(t.folio)}${t.cargoAutomatico ? " · cargo automático" : ""}
      </div>
    </button>`;
  }
  ...
  async function cargarOperaciones() {
    ...
    try {
      ...
      if (e.message !== "sesion")
        cont.innerHTML = '<div class="estado">No fue posible cargar la lista. Toca Operaciones para reintentar.</div>';
    }
  }
  $("buscar").addEventListener("input", () => {
    clearTimeout(temporizador);
    temporizador = setTimeout(cargarOperaciones, 350);
  });
    ...
    try {
      ...
      $("detalle").innerHTML = `
        ...
        <details class="bloque"><summary>El cobro</summary><div class="cuerpo">
          ${dato("Cobro", d.cobro.cargoAutomatico ? "Cargo automático autorizado" : "Sin cargo automático")}
          ${dato("Fechas de cargo", esc(d.cobro.fechasCargo))}
          ${dato("Cargos fallidos", d.cobro.cargosFallidos ? String(d.cobro.cargosFallidos) : "")}
          ${d.cobro.escrowEstado !== "No aplica" ? dato("Escrow", esc(d.cobro.escrowEstado + " · " + d.cobro.escrowMonto)) : ""}
        </div></details>
  ...
  async function cargarVencimientos() {
    ...
    try {
      ...
      const proximas = vivas.filter(t => t.semaforo.clave !== "vencida" &&
        t.semaforo.clave !== "disputa" && t.vencimiento && (t.semaforo.diasRestantes ?? 99) <= 30);
      const rango = (min, max) => proximas.filter(t =>
        t.semaforo.diasRestantes >= min && t.semaforo.diasRestantes <= max);
      ...
      const html =
        ...
        bloque("Esta semana", rango(0, 7)) +
        bloque("En 8 a 14 días", rango(8, 14)) +
        bloque("En 15 a 21 días", rango(15, 21)) +
        bloque("En 22 a 30 días", rango(22, 30));
      cont.innerHTML = html || '<div class="estado">Nada vence en los próximos 30 días y no hay saldos vencidos.</div>';
      cont.querySelectorAll(".tarjeta").forEach(b =>
        b.addEventListener("click", () => abrirDetalle(b.dataset.folio)));
    } catch (e) {
      if (e.message !== "sesion")
